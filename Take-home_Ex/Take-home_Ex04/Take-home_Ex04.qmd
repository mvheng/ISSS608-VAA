...
::: callout-note
Note that there is an increasing trend in the temperature over the years.
:::
...
Temp <- Temp_YM %>%     
   group_by(Date) %>%     
   summarise(MeanTemp = mean(AveMeanTemp)) 
...
::: callout-note
Note that there is an increasing trend in the total rainfall over the years.
:::
...
Rain <- Rain_YM %>%   
   group_by(Date) %>%       
   summarise(TotalRain = sum(TotRainfall))
