...
For this exercise, 4 network data modeling and visualisation packages will be installed and launched. They are `igraph`, `tidygraph`, `ggraph` and `visNetwork`. Besides these four packages, `tidyverse` and [`lubridate`](https://lubridate.tidyverse.org/), an R package specially designed to handle and wrangle time data will be installed and launched too.
...
we use [`theme_graph()`](https://ggraph.data-imaginist.com/reference/theme_graph.html) to remove the x and y axes.
...
- The argument `smooth` is used to plot the edges using a smooth curve.
